...
const stateObject : Item[] = [];

const itemName : HTMLInputElement = document.querySelector('#txtCardTitle')!;
const itemDescription : HTMLInputElement = document.querySelector('#txtCardDescrip')!;
...
const createCardItem = (item : Item) => {
...
const appendCard = (isDone : boolean) => {
  if (itemName.value) {
    const newItem = {
      name: itemName.value,
...
const useLocalStorage = () => {
  ...
  if (wishlist) {
    JSON.parse(wishlist).forEach((item : Item) => {
